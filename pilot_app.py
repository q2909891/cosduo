    ...
    with tabs[0]:
        ...
        with col_result:
            if not uploaded:
                ...
                st.info(
                    "**신뢰도란?** ResNet50 모델이 6개 질환 클래스에 대해 계산한 Softmax 확률 중 "
                    "가장 높은 값입니다. 예를 들어 87%라면 모델이 해당 질환일 확률을 87%로 추정한 것입니다. "
                    "단, 신뢰도가 높아도 학습 데이터 분포 밖의 이미지(측면 얼굴, 조명 불량 등)에서는 "
                    "결과가 부정확할 수 있으며, 본 결과는 임상 진단을 대체하지 않습니다.",
                    icon="ℹ️",
                )
